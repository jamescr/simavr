...
int main(int argc, char *argv[])
{
    ...
    // testing an elf format file.
    const char * fname =  "asm/01-led-parpadeo.elf";
    //char path[256];
...
//  printf("Firmware pathname is %s\n", path);
    ...
    // estas lineas deben estar aca porque en la funcion anterior pasa los
    // argumentos que encuentra en el elf apuntado por fname a la estructura f.
    uint32_t ffrequency = 16000000;
    f.frequency = ffrequency;
    printf("-- frequency %d y f.frequency %d\n", ffrequency, (int)f.frequency);

    char fmmcu[64] = "atmega328p";
    strncpy(f.mmcu, fmmcu, 64);
    printf("-- fmmcu %s y f.mmcu %s\n", fmmcu, f.mmcu);
    ...
    window = glutCreateWindow("Glut - Prueba");  /* create window */
